...
# Load a pre-trained count tokenizer
with open("tokenizer.pkl",'rb') as f:
    tokenizer = pickle.load(f)
# Load the tokenized array of queries
...
if prompt:
    ...
    print("LLM Response for valid query:", valid_q)
    ...
    if query_lis.__contains__(valid_q):
        ...
        #else:
            # use cv to get the similar index
            #idx = get_most_sim_idx(tokenizer.transform([prompt]), q_embs)
            #if idx is None:
            #    response = {"answer": "No solution found for the Given Query! Try Any Other Query"}
            #else:
                #response = qna[idx]['response']
    else:
        # print("Checking with LLM Agent for Response...")
        response = {}
        # response["answer"] = get_agent_response(prompt)
